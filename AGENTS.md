...
- **Book a Call → Calendly.** `contexts/BookingContext.jsx` provides `openBooking` / `closeBooking`. `BookingModal` lives in `app/layout.js` (outside `page-content`) so it overlays everything. It renders the Calendly booking iframe (`calendly.com/kjahstudio-support/30min?hide_gdpr_banner=1`) — a `.loader` spinner shows until `onLoad`, then the iframe fades in (`.frameHidden` → `.frameVisible`). Close button auto-focuses via `closeRef`; Esc + backdrop click close it. The **Nav CTA ("Book a Call")** and the **CTA section ("Book a Free Call")** both call `openBooking()`.
  - **⚠️ Dark-mode is a CSS invert hack, not native.** Calendly's free plan **ignores** the `background_color`/`text_color`/`primary_color` URL params (verified: the booking page renders white) and won't let you remove the "Powered by Calendly" badge — both are paid features. So `.frame` applies `filter: invert(1) hue-rotate(180deg)` to fake a dark theme over Calendly's light page (`hue-rotate` keeps blues/cyans roughly true). **Trade-offs:** the KJAH avatar + any imagery render inverted, and the badge is only recolored, not hidden. If the account ever upgrades to a paid Calendly plan, **remove the filter** and restore the color params instead — they'll then work natively.
